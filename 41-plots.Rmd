...
<a href="figures/03_pdf/r-graphical-parameters-cheatsheet.pdf" target="_blank"> 
```{r, echo=FALSE, out.width='50%'}
knitr::include_graphics('figures/03_pdf/r-graphical-parameters-cheatsheet.JPG')
```
</a>
...
text(1,3,pos=4, family = "Ubuntu mono", "(Ubuntu) mono")
```
...
## Graphics Size and Resolution

### Terminology

* [Point (typography)](https://en.wikipedia.org/wiki/Point_(typography)) Unit of length in typography. 1 point = $1/72$ inch $\approx$ 0.353 mm.
  - Typically, font size and size of spaces are defined in unit "pointsize"
* [Pixels per inch](https://en.wikipedia.org/wiki/Pixel_density) (ppi) measurement of pixel density 
...
* `pdf()`
  - `width`/`height`
    + By default 7 inches
    + Are always specified in **inches**. No `unit` argument.
    + To specify in cm use function `cm()` which converts **from** cm **to** inches.
  - No resolution argument necessary (vector graphic)
* `png()`
  - `width`/`height`
    + By default 480 units (by default pixels)
    + `unit` argument changes unit (`px`, `in`, `cm`, `mm`)
  - Resolution: See below

### Resolution and Font Size with png

Depends if `height`/`width` is specified in pixels or in length

* `unit = "px"` (NOT recommended)
   - `res` by default 72 ppi if not specified otherwise
   - pixels $\xrightarrow{\text{ppi}}$ inches
   - Changing `res` will also change the font size (relative to image dimensions)! Reason: `pointsize` is defined in inches and inches are defined via `ppi`.
* `height`/`width` not specified in pixels
  - `res` has to be specified (NO default value)
  - pixels $\xleftarrow{\text{ppi}}$ inches
  - Changing the `res` has no effect on font size (relative to image dimensions)
...
* Add two more digits to hexadecimal code, which define transparency
    - `#RRGGBBaa`
* Use `adjustcolor()` function
  - Alternative: `ggplot2::alpha()`

```{r, eval=FALSE}
plot(..., col = adjustcolor("gray", alpha.f = 0.5))
```
...
cdplot(fail ~ temperature, main = "cdplot()")
...
for(i in type_letters){
  ...
  text(x = 2, y = 8, labels = i, cex = 3, family = "Ubuntu mono")  # otherwise use "mono"
...
### Example: image and contour plot
...
* Cartesian convention
  1. matrix is transposed
    - rows of matrix end up on x axis
    - columns on y axis
  2. columns are flipped (first column is at bottom)
* European reading order
  - Matrix plotted as it would also be written down

For details see @baddeleySpatialPointPatterns2016 [chapter 3.6.3]
...
For European reading order first rotate matrix using `t(m[nrow(m):1,,drop=FALSE])` (see [here](https://stackoverflow.com/a/16497058/6152316))
...
Separately give data points for each box

```{r}
set.seed(1)
boxplot(rnorm(10, 4), rnorm(10, 6), rnorm(10, 9))
```

Specify assignment to box with formula
