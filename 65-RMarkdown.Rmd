...
* This is controlled by `knitr`
* Different ways for `.Rmd` and `.Rnw` documents
* `.Rmd`
  - For rmarkdown, the syntax highlighting is provided by pandoc, NOT `knitr.`
  - Depends if it is code block (no curly braces at top) or code chunk (curly braces at top)
    + For code blocks (without curly braces) everything is handled by pandoc
    + to check which languages are supported go to terminal and type `pandoc --list-highlight-languages`
    + Supported is e.g. R, bash, latex, bibtex, makefile, python but for example NOT stan.
  - [Manual](https://bookdown.org/yihui/rmarkdown/html-document.html#appearance-and-style)
...
```
  ...
  - Possible themes include: `default`, `tango`, `pygments`, `kate`, `monochrome`, `espresso`, `zenburn`, `haddock`, `breezedark`, and `textmate`
  - [Preview all themes](https://www.garrickadenbuie.com/blog/pandoc-syntax-highlighting-examples/)
  - [Modify themes](https://community.rstudio.com/t/edit-tango-highlight/39106) (not so easy!)
* `.Rnw`
  - syntax highlighting is provided by `knitr`
  - [Manual](https://rdrr.io/cran/knitr/man/knit_theme.html)
  - [Preview all themes](https://gist.github.com/yihui/3422133)
  - [Modify themes](https://stackoverflow.com/questions/11421614/code-styling-for-black-and-white-documents)
    + put `knit_theme$set("~/texmf/RetoKnitrCodeHighlight.css")` in the first code chunk and 
    + create/modify `RetoKnitrCodeHighlight.css`
...
Plots can be placed on right side with text wrapped around it. Use the following chunk options:
...
* For data in R use
  - `knitr::kable()`/[`kableExtra::kbl()`]((https://cran.r-project.org/web/packages/kableExtra/vignettes/awesome_table_in_html.html))
  - [`flextable`](https://ardata-fr.github.io/flextable-book/)
  - [`gt`](https://gt.rstudio.com/articles/intro-creating-gt-tables.html)
* Quick coercion of complex objects (fitted model, anova table ...) to Latex/HTML tables: [xtable](https://cran.r-project.org/web/packages/xtable/vignettes/xtableGallery.pdf)
...
pipe tables will always be full width in html and not full width in pdf. To make them a specific width in html use
